package com.bluedragonmc.example.game
...
// Learn more about this game at https://developer.bluedragonmc.com/intro/example-game/
/**
...
class ExampleGame(mapName: String) : Game(name = "ExampleGame", mapName = mapName) {
    ...
    override fun initialize() {
        ...
        use(ConfigModule()) // Needed for ConfigSpawnpointProvider used below
        use(SpawnpointModule(SpawnpointModule.ConfigSpawnpointProvider(allowRandomOrder = true))) // Spawns players in valid positions when they join the game (based on config.yml in the map folder)
        use(MOTDModule(Component.text("Here is a short explanation of how to play the game.\nCrouch to win!"), showMapName = true)) // Displays a welcome message to players joining the game.
